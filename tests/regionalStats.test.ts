...
import { terrainBand, terrainStep, buildRegionRows, kstToday, periodStart, validateDateRange } from '../src/components/statistics/regionalStats.ts';
...
test('지형 색상은 0건을 구분하고 범례 경계와 일치한다', () => {
  const step = terrainStep(40);
  assert.equal(step, 10);
  assert.deepEqual([0, 1, 9, 10, 19, 20, 29, 30, 40].map(n => terrainBand(n, step)), [0, 1, 1, 2, 2, 3, 3, 4, 4]);
  assert.equal(terrainBand(0, terrainStep(0)), 0);
});
test('긴 기간의 큰 건수에도 색상 범례를 확대해 지역 간 차이를 유지한다', () => {
  const step = terrainStep(1200);
  assert.equal(step, 300);
  assert.deepEqual([0, 30, 600, 780, 1200].map(n => terrainBand(n, step)), [0, 1, 3, 3, 4]);
});
