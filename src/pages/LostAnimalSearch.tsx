...
const fieldClass = 'w-full rounded-xl border border-border-light bg-white p-3 text-sm text-text-light focus:outline-none focus:ring-2 focus:ring-brand-focus dark:border-border-dark dark:bg-card-dark dark:text-text-dark';
const secondary = 'inline-flex min-h-11 items-center justify-center rounded-xl border border-border-light px-4 py-2 text-sm font-bold hover:bg-brand/10 focus-visible:outline-brand-focus dark:border-border-dark';
...
export default function LostAnimalSearch() {
  ...
  return <div className="min-h-screen bg-background-light text-text-light dark:bg-background-dark dark:text-text-dark">
    ...
    <main className="container mx-auto px-4 pb-16 pt-8 md:pt-12">
      ...
      <p className="mb-3 text-xs font-bold text-brand-accent">실종동물 찾기</p>
      ...
      <p className="mb-6 mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">사진이 닮아도 다른 동물일 수 있어요. 발견일·장소와 몸의 무늬를 확인한 뒤 보호소에 문의해 주세요.</p>
            <fieldset className="mb-8 max-w-[400px]"><legend className="mb-3 text-sm font-bold">어떤 동물을 찾고 계세요?</legend>
              <div className="grid grid-cols-2 gap-3">{(['DOG', 'CAT'] as const).map(value => <label key={value} className={`flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border text-sm font-bold ${species === value ? 'border-brand-focus bg-brand text-text-light' : 'border-border-light dark:border-border-dark'}`}>
                <input type="radio" name="species" value={value} checked={species === value} onChange={() => { invalidate(); setSpecies(value); }} className="accent-brand-ink" />
                {value === 'DOG' ? '강아지' : '고양이'}
              </label>)}</div>
            </fieldset>
      <div className="grid items-start gap-8 lg:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)] xl:gap-10">
        <section aria-labelledby="input-heading" className="min-w-0">
          ...
          <form onSubmit={search} className="space-y-5">
            ...
            <div className="rounded-2xl border border-border-light bg-brand-soft p-4 dark:border-border-dark dark:bg-card-dark">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border-light p-4 dark:border-border-dark">
              <input type="checkbox" aria-label="입양·반환된 동물도 포함" checked={includeAdoptedOrReturned} onChange={e => { invalidate(); setIncludeAdoptedOrReturned(e.target.checked); }} className="mt-0.5 rounded border-gray-300 text-brand-ink focus:ring-brand-focus" />
            ...
            {phase === 'loading' ? <button key="cancel" type="button" onClick={event => { event.preventDefault(); invalidate(); }} className={`${secondary} w-full`}>검색 취소</button> : <button key="submit" type="submit" disabled={!photo || validating} className="min-h-12 w-full rounded-xl bg-brand px-4 py-3 text-sm font-bold text-text-light disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-500 focus-visible:outline-brand-focus">{phase === 'error' ? '다시 시도하기' : '사진으로 후보 찾기'}</button>}
        ...
        <section aria-labelledby="result-heading" aria-busy={phase === 'loading'} className="min-w-0">
          ...
          <div aria-live="polite">
            {phase === 'loading' && <div className="rounded-2xl border border-border-light p-8 text-center dark:border-border-dark"><span className="mx-auto mb-4 block h-9 w-9 animate-spin rounded-full border-4 border-brand-focus border-t-brand-focus motion-reduce:animate-none" /><p className="font-bold">닮은 동물을 찾고 있어요</p><p className="mt-2 text-sm text-gray-500">사진과 공고 정보를 확인하고 있어요. 잠시만 기다려 주세요.</p></div>}
            {phase === 'idle' && <div className="rounded-2xl border border-dashed border-brand-focus bg-brand-soft p-8 text-center dark:bg-card-dark"><p className="font-bold">사진을 선택하고 후보를 찾아보세요</p><p className="mt-3 text-sm leading-6 text-gray-600 dark:text-gray-300">기본 검색은 현재 공고·보호 중인 동물을 확인해요.<br />사진이 닮아도 다른 동물일 수 있어요.</p></div>}
          ...
          {phase === 'success' && results.length > 0 && <>
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{results.map(({ animal, shelterPhone, matchedEvidence }) => <article key={animal.id} className="min-w-0 overflow-hidden rounded-2xl border border-border-light bg-card-light dark:border-border-dark dark:bg-card-dark">
              ...
              <div className="space-y-3 p-4"><p className="text-xs font-bold text-emerald-700 dark:text-brand-accent">{statusLabels[animal.status] || '상태 확인 필요'}</p>
                ...
                {matchedEvidence.filter(e => evidenceLabels[e]).length > 0 && <ul className="flex flex-wrap gap-1.5">{[...new Set(matchedEvidence)].filter(e => evidenceLabels[e]).map(e => <li key={e} className="rounded-lg bg-brand/10 px-2 py-1 text-xs leading-5">{evidenceLabels[e]}</li>)}</ul>}
                ...
                {shelterPhone && /^\+?[0-9 ()-]{5,30}$/.test(shelterPhone) && <a href={`tel:${shelterPhone.replace(/[^+0-9]/g, '')}`} className="block py-2 text-center text-sm font-medium text-brand-accent underline">보호소 문의 · {shelterPhone}</a>}
              </div>
            </article>)}</div>
          </>}
        </section>
