...
const focus = 'focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-ink dark:focus-visible:outline-brand';
const textLink = `inline-flex min-h-11 items-center text-sm font-bold text-brand-ink hover:underline dark:text-brand ${focus}`;
const selectStyle = 'h-11 w-full min-w-0 rounded-lg border-brand-border bg-white text-base text-brand-ink focus:border-brand-ink focus:ring-brand-ink dark:border-border-dark dark:bg-card-dark dark:text-gray-200';
      ...
      <main className="container mx-auto w-full flex-1 px-4 pb-12 pt-7 md:pb-16 lg:pt-14">
        <section aria-labelledby="home-heading" className="grid items-stretch gap-6 lg:grid-cols-[1.15fr_1fr] lg:gap-8">
          <div className="min-w-0">
            ...
            <p className="mt-5 hidden text-base leading-relaxed text-brand-muted dark:text-gray-300 sm:block">
              새로운 가족을 기다리는 동물들.<br />지역과 동물 종류로 나에게 맞는 만남을 찾아보세요.
            </p>
            <form aria-label="보호 동물 빠른 검색" onSubmit={search} className="mt-5 rounded-xl border border-brand-border p-4 dark:border-border-dark lg:mt-6">
              ...
              <button type="submit" className={`mt-4 min-h-12 w-full rounded-lg bg-brand px-4 py-3 text-sm font-bold text-brand-ink transition-colors hover:bg-brand-hover ${focus}`}>동물 찾아보기</button>
            </form>
          </div>
        ...
        <section aria-labelledby="animals-heading" className="mt-12 lg:mt-14">
          ...
          {animals.isPending ? <p role="status" className="py-12 text-brand-muted dark:text-gray-300">보호 동물을 불러오고 있어요.</p>
            : animals.isError && !animals.data ? <div role="alert" className="rounded-xl border border-brand-border p-6 dark:border-border-dark"><p>보호 동물을 불러오지 못했어요.</p><button onClick={() => void animals.refetch()} className={textLink}>다시 불러오기</button></div>
              : featuredAnimals.length === 0 ? <p className="py-12 text-brand-muted dark:text-gray-300">현재 표시할 보호 동물이 없습니다.</p>
        ...
        <section aria-labelledby="stories-heading" className="mt-12">
          ...
          {posts.isPending ? <p role="status" className="py-6 text-brand-muted dark:text-gray-300">입양 이야기를 불러오고 있어요.</p>
            : posts.isError && !posts.data ? <div role="alert"><p>입양 이야기를 불러오지 못했어요.</p><button onClick={() => void posts.refetch()} className={textLink}>후기 다시 불러오기</button></div>
              : stories.length === 0 ? <p className="py-6 text-brand-muted dark:text-gray-300">아직 입양 후기가 없습니다. <Link to="/adoption" className={textLink}>입양 후기 보기</Link></p>
                : <div className="grid gap-4 md:grid-cols-2">{stories.map(post => (
                  <Link key={post.postId ?? post.id} to={`/adoption/${post.postId ?? post.id}`} className={`group rounded-2xl border border-brand-border p-6 transition-colors hover:border-brand-ink dark:border-border-dark ${focus}`}>
                    <p className="text-xs text-brand-muted dark:text-gray-400">입양 후기 · {new Date(post.createdAt).toLocaleDateString('ko-KR')}</p>
                    <h3 className="mt-3 break-words text-lg font-bold group-hover:underline">{post.title}</h3>
                    <p className="mt-3 line-clamp-2 break-words text-sm leading-relaxed text-brand-muted dark:text-gray-300">{post.content.replace(/\s+/g, ' ').trim()}</p>
                    <span className="mt-4 inline-block text-sm font-bold text-brand-ink dark:text-brand">이야기 읽기 →</span>
                  </Link>
                ))}</div>}
        </section>

        <section aria-labelledby="stats-heading" className="mt-12 rounded-2xl bg-brand-soft p-5 dark:bg-neutral-800 sm:p-6">
          ...
          <dl className="mt-5 grid grid-cols-3 gap-3 sm:gap-6">
            ...
            ].map(([label, value, unit]) => <div key={label}><dt className="text-xs leading-relaxed text-brand-muted dark:text-gray-300 sm:text-sm">{label}</dt><dd className="mt-2 text-xl font-bold sm:text-3xl">{value}<span className="ml-1 text-sm font-medium">{unit}</span></dd></div>)}
          </dl>
          <p className="mt-5 text-xs leading-relaxed text-brand-muted dark:text-gray-400">{today} 기준 · 비율은 최근 30일 구조 동물 전체의 현재 상태 기준입니다.</p>
        ...
        <nav aria-label="더 둘러보기" className={`mt-12 grid divide-y divide-brand-border rounded-xl border border-brand-border dark:divide-border-dark dark:border-border-dark ${showPetMarket ? 'sm:grid-cols-3' : 'sm:grid-cols-2'} sm:divide-x sm:divide-y-0`}>
          {[['/adoption', '입양 후기', '가족이 된 이후의 이야기'], ['/community', '커뮤니티', '궁금한 점을 함께 나눠요'], ...(showPetMarket ? [['/products', '펫마켓', '반려생활에 필요한 것들']] : [])].map(([to, title, description]) => (
            <Link key={to} to={to} className={`flex min-h-16 flex-wrap items-center gap-x-4 gap-y-1 p-4 hover:bg-brand-soft dark:hover:bg-neutral-800 sm:block ${focus}`}><span className="text-sm font-bold">{title}</span><span className="text-xs text-brand-muted dark:text-gray-400 sm:mt-2 sm:block">{description}</span></Link>
          ))}
        </nav>
      </main>
      <Footer />
