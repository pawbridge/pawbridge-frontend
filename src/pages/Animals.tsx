      ...
      <main className="mx-auto w-full max-w-[1504px] px-4 pb-14 pt-10 md:px-6 md:pb-20 md:pt-14">
        ...
        <section aria-labelledby="animal-search-results" className="mt-10">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 id="animal-search-results" className="text-xl font-bold text-brand-ink dark:text-text-dark md:text-2xl">검색 결과 {totalElements.toLocaleString()}마리</h2>
              <p className="mt-1 text-xs text-brand-muted dark:text-gray-300 md:text-sm">{filters.noticeNo ? '입력한 공고번호와 정확히 일치하는 결과예요.' : relevanceSearch ? '입력한 특징과 가까운 순서로 보여드려요.' : '최근 접수된 동물부터 보여드려요.'}</p>
            </div>
          ...
          {isLoading && !data ? (
            <div aria-label="동물 검색 결과를 불러오는 중" className="grid gap-4 lg:grid-cols-2">
              {[0, 1, 2, 3].map((item) => <div key={item} className="h-[236px] animate-pulse rounded-2xl border border-brand-border bg-stone-50 dark:bg-stone-900 md:h-[252px]" />)}
            </div>
          ) : error ? (
            <div role="alert" className="rounded-2xl border border-[#f1c6c6] bg-[#fff8f8] dark:bg-red-950/20 px-6 py-12 text-center">
              <h3 className="text-lg font-bold text-[#7f1d1d] dark:text-red-300">검색 결과를 불러오지 못했습니다</h3>
              <p className="mt-2 text-sm text-brand-muted dark:text-gray-300">잠시 후 다시 시도해 주세요.</p>
              <button type="button" onClick={() => void refetch()} className="mt-5 h-11 rounded-xl bg-brand-ink px-6 text-sm font-bold text-white">다시 시도</button>
            </div>
          ) : animals.length === 0 ? (
          ...
          {totalElements > maxAnimalSearchResults && <p className="mt-6 text-center text-xs leading-5 text-brand-muted dark:text-gray-300">공고 정보를 안정적으로 제공하기 위해 앞의 {maxAnimalSearchResults.toLocaleString()}마리까지 볼 수 있습니다. 조건을 좁히면 원하는 동물을 더 빠르게 찾을 수 있어요.</p>}
